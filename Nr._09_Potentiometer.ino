...
/*
 * Der Loop-Teil wird nun erneut gestartet. Wenn sich der Wert des ausgelesenen Drehreglers ändert, dann ändert sich auch die Zeit zwischen den Ein- und Aus-Phasen der LED.
 * Das Blinken wird dadurch schneller und langsamer. Das längste delay beträgt in diesem Sketch 1023ms (Millisekunden).
 * Wenn man längere delays benötigt, dann baut man eine kleine mathematische Zeile in den Code ein.
 * Beispiel: Man ändert die Zeile „sensorwert =analogRead(eingang);“ in „sensorwert =analogRead(eingang)*2;“
 * Damit wird der abgespeicherte Sensorwert um den Faktor 2 vergrößert. Da längste delay wäre dann 2046ms usw…
 */
